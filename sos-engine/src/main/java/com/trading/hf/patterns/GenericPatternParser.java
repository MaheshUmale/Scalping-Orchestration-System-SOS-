...
public class GenericPatternParser {
    ...
    public Map<String, PatternDefinition> loadPatterns(String directoryPath) {
        ...
        try (InputStream in = getClass().getClassLoader().getResourceAsStream(directoryPath);
             BufferedReader br = new BufferedReader(new InputStreamReader(in))) {

            String resourceName;
            while ((resourceName = br.readLine()) != null) {
                if (resourceName.endsWith(".json")) {
                    try (InputStream jsonStream = getClass().getClassLoader().getResourceAsStream(directoryPath + "/" + resourceName)) {
                        ...
                        log.error("Error parsing pattern file: {}", resourceName, e);
                    }
                }
            }
        } catch (IOException e) {
